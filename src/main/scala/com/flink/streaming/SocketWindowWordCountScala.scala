...
object SocketWindowWordCountScala {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //链接socket获取输入数据
    val text: DataStream[String] = env.socketTextStream("zytc221",port,'\n')
    ...
    val windowCounts = text.flatMap(line => line.split(" "))//打平，把每一行单词都切开
      .map(w => WordWithCount(w,1))//把单词转成word , 1这种形式
      .keyBy("word")//分组
      .timeWindow(Time.seconds(10))//指定窗口大小，指定间隔时间
