...
object SocketDemoMysql {
    ...
    class myMysql extends RichSinkFunction[Tuple2[String, Int]] {
        ...
        override def open(parameters: Configuration): Unit = {
            ...
            val conn: Connection = DriverManager.getConnection(url, user, pswd)
            val sql:String = "insert into wordcount(word,count) values(?,?)"
            val ppst: PreparedStatement = conn.prepareStatement(sql)
        }
